...
def get_missing_tags() -> list[str]:
    all_tags = get_all_tags()
    existing_tags = get_existing_tags()
    return sorted(set(all_tags) - set(existing_tags))


def get_all_tags() -> list[str]:
    all_versions = _get_node_package_versions(PACKAGE_NAME)
    all_versions = [v for v in set(all_versions) if "nightly" not in v]
    return sorted(f"v{v}" for v in all_versions)


def get_existing_tags() -> list[str]:
    tags = git("tag", "--list").splitlines()
    return sorted(t for t in set(tags) if t.startswith("v"))
